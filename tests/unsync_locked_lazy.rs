...
use static_init::{UnSyncLockedLazy,dynamic, Phase};
...
fn normal() {
    ...
    spawn(|| assert_eq!(*NORMAL.read(), vec![1,2])).join().unwrap();
}
...
fn init_may_panick() {
    ...
    assert_eq!(INIT_MAY_PANICK.phase(), Phase::INITIALIZATION_PANICKED | Phase::INITIALIZATION_SKIPED);
...
fn init_may_panick_intolerant() {
    ...
    assert_eq!(UNINITIALIZABLE.phase(), Phase::INITIALIZATION_PANICKED | Phase::INITIALIZATION_SKIPED);
